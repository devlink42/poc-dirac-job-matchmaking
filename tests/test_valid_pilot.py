...
def load_node(node_path: str) -> Node:
    """Load a Node object from a YAML file."""
    with open(node_path, "r") as f:
        data = yaml.safe_load(f)
    # Add a dummy node_id if missing for validation
...
def load_job_specs(job_path: str) -> list[Job]:
    ...
    for i, spec in enumerate(data.get("matching_specs", [])):
        # Add a dummy job_id if missing for validation
...
@pytest.mark.parametrize("job_file, node_file, expected_match", [
    ...
    ("tests/examples/jobs/job_01_mcsimulation_any_site.yaml",
     "tests/examples/nodes/pilot_02_tier2_older.yaml", False),
    # RAM too small (Job 1.5GB vs Node 1GB)
    ("tests/examples/jobs/job_01_mcsimulation_any_site.yaml",
     "tests/examples/nodes/pilot_04_low_ram.yaml", False),
    # Missing GPU on node
    ("tests/examples/jobs/job_06_gpu.yaml",
     "tests/examples/nodes/pilot_01_cern_typical.yaml", False),
...
def test_matchmaking_combinations(job_file, node_file, expected_match):
